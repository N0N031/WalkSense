// src/services/sessionService.ts (CORRIGÉ — ajout exportSessionPdf)
...
class SessionService {
  ...
  async resumeSession(): Promise<Session | null> {
    try {
      const session = await this.getCurrentSession();
      if (!session) return null;
      const updated: Session = { ...session, status: "running" };
      await sessionRepository.updateSession(updated);
      return updated;
    } catch (error) {
  ...
  async endSession(
    ...
    try {
      ...
      const now = Date.now();
      const canonical = this.buildCanonical(session);
  ...
  async addGpsPoint(sessionId: string, point: GpsPoint): Promise<boolean> {
    try {
      const session = await this.getSessionById(sessionId);
      if (!session) return false;

      // Éviter les doublons
      const alreadyExists = session.gpsTrace.some(
        (gp) =>
          gp.timestamp === point.timestamp &&
          gp.lat === point.lat &&
          gp.lon === point.lon,
      );
      ...
      const updated: Session = {
        ...session,
        gpsTrace: [...session.gpsTrace, point],
      };
  ...
  async addEvent(sessionId: string, event: MarkedEvent): Promise<boolean> {
    try {
      const session = await this.getSessionById(sessionId);
      if (!session) return false;
  ...
  async updateEvent(sessionId: string, event: MarkedEvent): Promise<boolean> {
    try {
      const session = await this.getSessionById(sessionId);
      if (!session) return false;
      ...
      const updated: Session = {
        ...session,
        events: session.events.map((e) => (e.id === event.id ? event : e)),
      };

      await sessionRepository.updateSession(updated);
      return true;
    } catch (error) {
      console.error("SessionService.updateEvent error:", error);
      return false;
    }
  }

  async refillEvent(sessionId: string, eventId: string): Promise<boolean> {
    try {
      const session = await this.getSessionById(sessionId);
      if (!session) return false;
  ...
  async classifyEvent(
    ...
    try {
      ...
      const updated: Session = {
        ...session,
        events: session.events.map((e) =>
          e.id === eventId ? { ...e, classification } : e,
        ),
      };
...
`;
  ...
  async exportSessionPdf(
    ...
    try {
      ...
      const filepath = await pdfService.exportSessionPdf(session, {
      ...
      console.error("SessionService.exportSessionPdf error:", error);
      throw error;
    }
  }
